    ...
    <section v-reveal class="home-brand">
        <div class="container">
            <div class="home-brand__stage">
                <nuxt-link v-if="cover" class="home-brand__photo" to="/brand/curacion">
                    <img
                        :src="cover"
                        :alt="brandName"
                        width="1200"
                ...
                <div class="home-brand__copy">
                    ...
                    <nuxt-link class="home-brand__about" to="/brand/curacion">О бренде</nuxt-link>
                </div>
            </div>
            ...
            <p class="home-brand__more">
                <nuxt-link to="/brand/curacion">Все товары {{ brandName }}</nuxt-link>
            </p>
        </div>
    </section>
</template>
...
const { data: brand } = await useAsyncData(
    'home-brand-curacion',
    () => $fetch(`${apiBase}/market/brands/curacion/`).catch(() => null),
);

const { data: goodsResponse, pending: goodsPending } = await useAsyncData(
    'home-brand-curacion-goods',
...
const brandName = computed(() => brand.value?.name || 'Curación');
const lead = computed(() => brand.value?.lead || '');
const cover = computed(() => brand.value?.hero || brand.value?.history_image || '');
</script>
